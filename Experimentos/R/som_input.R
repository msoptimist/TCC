...
#tamanho do grid
input.grid <- somgrid(x=16, y=16, topo="rectangular")

#executar SOM
set.seed(100)
input.som <- som(input.sc, grid=input.grid, rlen=500, alpha=c(0.5, 0.01))
...
#wss <- fviz_nbclust(getCodes(input.som), kmeans, method = "wss", k.max = 30)
#plot(wss)
